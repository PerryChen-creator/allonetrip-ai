...
⚠️ **地點區域約束與輸出規範（最高優先級）**：
1. **地點區域真實性**：使用者所在位置為 ${locationDesc}。請根據實際狀況推薦 1 ~ 3 個符合條件的地點。如果該範圍或類別極難找（例如平原區域找山脈），請據實說明並推薦最接近的替代方案，**絕對不要硬湊數量或輸出半途中斷**！
2. **完整輸出**：所有推薦的地點都必須包含完整細節，絕對不可中途截斷！
3. **格式要求**：
...
4. **結尾 CTA**：結尾附上一句親切的 Perry 獨旅溫馨提示，並鼓勵使用者點選下方輸入框帶入行程。`;
      ...
      try {
        const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            max_tokens: 2500, // 🟢 增加 Token 輸出上限，防止長文被半路截斷
          }),
        });

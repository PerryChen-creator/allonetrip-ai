...
2. **交通與火車／高鐵規範**：
   - 若為國內/城際交通（如火車、高鐵），必須提供建議車次型號與約略時間段（例如：建議搭乘台鐵自強號 109 次 08:00 台北發車 -> 10:15 抵達彰化）。
   - 國內火車請附上：\`[台鐵時刻查詢](https://www.railway.gov.tw/)\` 或 \`[台灣高鐵預訂](https://www.thsrc.com.tw/)\`。
   - ❌ **絕對禁止** 在火車、公車或國內交通上附加 Google Flights 機票連結！機票連結僅限跨國或離島搭飛機行程。

3. **🚫 嚴禁使用 HTML 標籤**：
   - ❌ 絕對禁止使用 \`<br>\` 等 HTML 標籤，表格儲存格內有多個項目請用「、」或「；」連接。
...
- 八卦山大佛 [📍 導航](https://www.google.com/maps/search/?api=1&query=八卦山大佛)｜搭乘計程車前往，參觀時間約 1 小時。

11:00 - 12:00：午餐美食
- 推薦餐廳：彰化肉圓 [📍 導航](https://www.google.com/maps/search/?api=1&query=彰化肉圓) 或 阿明米糕 [📍 導航](https://www.google.com/maps/search/?api=1&query=阿明米糕)。
--------------------------------------------------
`;
